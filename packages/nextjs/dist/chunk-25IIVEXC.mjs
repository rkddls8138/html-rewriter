...
async function fetchAndGenerateMetadata(config, path, options) {
  const tags = await fetchSeoMeta(config, path, {
    noCache: options?.noCache
  });
  return generateSeoMetadata(tags);
}
function createMetadataFetcher(config) {
  return (path, options) => fetchAndGenerateMetadata(config, path, options);
}
