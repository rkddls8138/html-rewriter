...
var fetchSeoMetaForPages = (config, path, options) => fetchSeoMeta(config, path, options);
var createPagesMetaFetcher = (config) => (path, options) => fetchSeoMetaForPages(config, path, options);
var withSeoMeta = (config, getServerSideProps, getPath, options) => async (context) => {
  const [meta, result] = await Promise.all([
    fetchSeoMetaForPages(config, getPath(context), options),
